...
export const LlmCallCostRecordSchema = z
  .object({
    ...
    call_kind: z.enum(["llm", "stt", "tts"]).default("llm"),
    metering_unit: z.enum(["token", "audio_second", "character"]).default("token"),
    quantity: z.number().nonnegative().default(0),
    unit_price_per_million_usd: z.number().nonnegative().default(0),
    device_correlation_id: z.string().uuid().nullable().default(null),
