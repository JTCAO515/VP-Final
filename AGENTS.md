...
- Codex owns implementation, architecture review, and merge authority for this project. Every PR still
  requires an explicit Codex review pass after implementation, all required checks green, and the
  applicable merge checklist in `docs/governance/issue-pr-workflow.md`. Tier A is limited to pure
  documentation or draft ADRs, manifest registration, visual-only UI changes with no data-flow or
  clickable-behavior change, test-only/type-fix/dependency-alignment changes, and serialized handoff
  refreshes for merged work.
- Tier B includes identity/auth/permissions/RLS, payment/billing/ledger/cost paths,
  conversation/PII/retention/redaction, external service or safety promises, domain schemas or database
  migrations, AI pipeline invariants, and every D2/D3 Issue. It requires the fuller Codex architecture
  review described in the governance workflow before merge; it does not require a separate reviewer.
