# JTCoding Skills：VisePanda 软件系统工程方法

Status: active
Current skill id: `jtcoding-skills`
Former repository id: `qian-systems-engineering`
Applies to: product, architecture, code, data, AI, operations, commercial, brand, and marketing work

“JTCoding Skills”是 VisePanda 的永久工程工作流，也是原“钱学森 Skills”的正式命名和扩展
版本。它把总体目标、模块分解、接口、执行、观测、反馈、纠偏和知识归档连接成一个可审查的
闭环。它与现有文档即代码机制结合，保留当前 Fable-5 架构和已经接受的项目规则，不另造一套
平行管理体系。

本项目现在只使用 JTCoding Skills 这一个入口。它内部吸收三套互补方法：钱学森系统工程负责“为什么做、系统如何分解、如何
反馈纠偏”；Matt Pocock 公开工程实践启发的文档即代码负责“知识如何持久、索引与接手”；
Karpathy coding discipline 负责“本次代码如何保持简单、聚焦、可验证”。这些旧名称是 JTCoding
的内部来源和历史脉络，不再作为本项目需要单独调用的 skills。
...
的 MIT Skill，验证基线提交为 `2c606141936f1eeef17fa3043a72095b4765b9c2`。这些来源已经合并
进 JTCoding Skills。当前工作只触发 JTCoding；旧的 Qian、Matt-inspired 或 Karpathy skill 不再
单独作为项目入口。JTCoding 在这些实践上增加总体设计、控制量、偏差分级、生命周期门禁和综合
集成复盘。

| 层 | 控制对象 | 核心问题 | 主要产物 |
| --- | --- | --- | --- |
| JTCoding 系统工程层 | 整体系统与生命周期 | 目标是否正确、接口是否稳定、偏差如何纠正 | 总体设计、Issue、ADR、观测与复盘 |
| JTCoding 文档即代码层 | 工程知识与接手 | 事实是否可查、代码与文档是否同步 | manifest、Index、handoff、说明/约束文档 |
| JTCoding 聚焦实现层 | 单次实现与 review | 是否假设过多、过度设计、改动过宽、验收过弱 | 假设、最小方案、聚焦 diff、step-to-check 证据 |

优先级为：安全/权限/数据/支付/本地硬约束 → 已接受总体设计与接口 → Issue 范围与验收 →
JTCoding 生命周期与文档控制 → JTCoding 局部简化。简单不能成为绕过系统不变量的理由；系统工程
...
完整、规范性的条款位于
[JTCoding Skills 约束](../constraints/qian-systems-engineering.md)。以下为不可豁免的核心：
...
9. 每次编码必须遵守 [JTCoding Focused Implementation Discipline](../constraints/karpathy-guidelines.md)；
