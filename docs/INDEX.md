...
- **Updated:** 2026-08-11 by Codex / serialized post-#355 CI stability refresh
- **Base branch:** `main`
- **Last fully verified commit:** `73ea1b2 test(ci): make shared trigger cleanup deterministic; PR #355 head passed verify, Database contracts, evals, docs, and Vercel Preview Comments before authorized merge as d8101ea`
- **Current phase:** Phase 0 — production hardening and commercial safety
- **Maturity:** The real-provider Copilot, durable Trip completion, signed anonymous identity, cost and product-event persistence, three-turn and trusted-IP guards, truthful request states, controlled-preview Human Tasks, legal pages, retention contracts, database-local retention scheduler, durable outbound ledger, Admin-only partner configuration, P0-19 privacy-safe telemetry contract/adapter/views/producers/public-ingestion guard, and VisePod HTTPS Device Protocol v1 are merged. ADR-0013 freezes the Phase 0 Human Task rule: only durable intake may emit task_submitted; Ops triage/cancellation and quote/payment/done actions have no producer. The public telemetry endpoint now applies pre-write, HMAC-keyed dual identity/network Upstash limits and returns honest 429 or 503 outcomes. P0-20's ADR-0015 is a draft in independent Tier B review; runtime implementation must not begin until that review accepts or revises the contract. No partner is active, no booking or commission is promised, no payment is live, and Phase 1 through Phase 4 remain trigger-gated.
- **Last completed control action:** PR #355 removed a nondeterministic Database-contracts failure source without weakening coverage: the one test that creates and drops a shared temporary audit trigger now runs in a separate Vitest process after unrelated server tests. Its head passed verify, docs, evals, Database contracts, and both Vercel previews before merge as d8101ea. P0-19 remains terminal under ADR-0013; payment conversion remains deferred with P0-17 / OA-006. PR #328 froze the VisePod v1 Wi-Fi HTTPS turn contract; PR #329 accepted ADR-0013; P0-18 remains complete through PRs #309/#315/#317. Issue #311 independently verified the three production migrations, pg_cron schedule, and retention health on VP-Final-V2 without a production write.

### Active Work

| Ref | Work | State | Owner | Next action |
| --- | --- | --- | --- | --- |
| Issue #157 / P0-20 | Add truthful public-runtime safety and budget controls | P0-19 is terminal. PR #338 proposes ADR-0015, a Tier B contract for one coherent health, error, token-budget, and Human Help rate-control boundary that reuses existing Copilot and telemetry controls. Its independently verified CI checks are green, but it has not received independent architecture approval. | Independent architecture reviewer for ADR-0015; Codex implementation only after acceptance | Independently accept or revise ADR-0015 in PR #338. Do not create a P0-20 runtime implementation branch while the contract remains draft. |
...
| --- | --- | --- |
| P0 | Complete independent Tier B review of PR #338 / ADR-0015 before any #157 runtime implementation work. | ADR-0015 is accepted or revised and re-reviewed; only then may a fresh-main P0-20 implementation sequence add explicit health/degradation, safe error-observation, non-duplicative request/token and Human Help limits, documented authorized Ops exemptions, and truthful UI states. |
