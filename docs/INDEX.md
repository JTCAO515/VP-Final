...
| --- | --- | --- |
| P1 | Continue Issue #154 with the authorized Ops-only Checkout ledger writer, signature-verified webhook, and idempotent Human Task state synchronization from the merged payment ledger and guarded adapter. | Missing configuration remains unavailable; an authorized quoted task can receive one provider Checkout Session; only a verified, idempotent provider event can transition its matching task and ledger record to paid. No card data, key, payload, signature, or placeholder success is stored. |
...
- P0-17 payment code is in progress: its private ledger and guarded server-only Checkout adapter are merged, while the authorized Checkout writer, webhook verification, and Stripe configuration remain unimplemented. No payment, booking, commission, or paid Human Help capability is live.
