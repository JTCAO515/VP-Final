...
- **Updated:** 2026-08-13 by Codex / serialized post-#380 provider-diagnostic and #382 VisePanda workspace refresh
- **Base branch:** `main`
- **Last fully verified commit:** `d617ef17 feat(web): separate VisePanda workspace from product home; PR #382 head passed GitHub verify, evals, Database contracts, and Documentation Governance before merge`
- **Current phase:** Phase 0 — production hardening and commercial safety
- **Maturity:** The real-provider Copilot, durable Trip completion, signed anonymous identity, cost and product-event persistence, three-turn and trusted-IP guards, truthful request states, controlled-preview Human Tasks, legal pages, retention enforcement, durable outbound ledger, Admin-only partner configuration, P0-19 telemetry, and P0-20 public-runtime controls are merged. P0-20 preserves the existing anonymous and trusted-network guards, bounds Copilot input/output, adds the authenticated HMAC-keyed Upstash window and per-identity China-day Human Help cap, keeps daily cost observation non-blocking, and provides truthful recovery with safe correlation-only route logs. Execution-fact safety has a reviewed safe-phrase schema, fail-closed model bypass/grounding guard, and blocking runtime evals. The mandatory deterministic policy-eval suite now covers commerce intent, review-only Human Help handoff, invalid TripPatch rejection, insufficient-knowledge honesty, and medical/legal fixed-expression paths. Local-address facts, legacy-address quarantine, deterministic POI alias resolution, and the no-fallback presentation guard are also merged. The Web public home is now product-first while the live VisePanda workspace is canonical at /visepanda; /copilot redirects there, scenario-only context prefill stays editable and never auto-sends, and no booking, payment, Human Help submission, or Trip-editing capability was added. Provider readiness has a private secret-safe startup diagnostic and retains typed unavailable behavior instead of mock success. VisePod has the accepted HTTPS Device Protocol v1 plus a draft Studio provisioning contract and pure domain schemas only; no Studio route, token issuer, binding table, device registry, gateway, or firmware runtime exists. ADR-0015 and ADR-0016 are Accepted; ADR-0017 remains Draft. No partner is active, no booking or commission is promised, no payment is live, and Phase 1 through Phase 4 remain trigger-gated.
- **Last completed control action:** PR #382 completed Issue #381 after real GitHub verify, evals, Database contracts, and Documentation Governance checks succeeded. It separates the product-first public home from the live /visepanda workspace, redirects legacy /copilot, unifies user-visible VisePanda naming, and uses allowlisted editable scenario context without URL PII or auto-send. PR #380 completed Issue #72 with a private, secret-safe provider readiness diagnostic; it retains typed unavailable behavior and exposes no public provider-health endpoint or secret.

### Active Work

| Ref | Work | State | Owner | Next action |
| --- | --- | --- | --- | --- |
| ADR-0017 / Issues #335-#341 | VisePod Studio provisioning and binding sequence | PR #361 merged the draft contract and pure domain schemas; Issue #335 is closed. No persistence, provisioning token, endpoint, or Studio consumer is implemented, and ADR-0017 still declares Status: Draft. | Independent architecture reviewer for acceptance; Codex only after the accepted dependency is explicit | Accept or revise ADR-0017 before starting #336. If accepted, implement #336-#341 strictly serially from fresh main as Tier B work. |
| Issue #298 / VPOD-00 | Freeze multi-meter voice-call cost accounting contract | Implementation remains paused at a D2 schema boundary: the existing unique Agent Run attempt cannot represent STT+LLM+TTS rows, and per-device aggregation has no frozen opaque device-correlation contract. | Architecture owner for contract freeze; Codex implementation after acceptance | Accept or revise the proposed call kind, uniqueness, and device-correlation semantics in #298; then start one additive Tier B migration/ADR PR from fresh main. |
...
| --- | --- | --- |
| P0 | Observe the deployed P0-20 controls and the public VisePanda home/workspace split without weakening fail-closed behavior; record only sanitized deployment observations and do not describe browser behavior as live until it is observed. | The operator-action register contains required sanitized deployment observations; the /copilot redirect, /visepanda workspace, and mobile view switch have browser evidence without any public capability claim beyond what is implemented. |
| P0 | Accept or revise ADR-0017 before starting VisePod Studio runtime issues #336 through #341. | ADR-0017 is explicit Accepted or a replacement contract is merged; runtime work starts from fresh main under the accepted contract. |
...
- ADR-0017 remains Draft even though its bounded contract/schema PR merged. Independent architecture must accept or revise it before downstream VisePod Studio work treats it as authority.
