...
- **Updated:** 2026-08-12 by Codex / serialized post-#357-#367 and #361 safety, knowledge, and Studio-contract refresh
- **Base branch:** `main`
- **Last fully verified commit:** `264676c docs(visepod): freeze Studio binding contract; main CI run 31525051523 passed verify, evals, and Database contracts, and Documentation Governance passed on the merge commit`
- **Current phase:** Phase 0 — production hardening and commercial safety
- **Maturity:** The real-provider Copilot, durable Trip completion, signed anonymous identity, cost and product-event persistence, three-turn and trusted-IP guards, truthful request states, controlled-preview Human Tasks, legal pages, retention enforcement, durable outbound ledger, Admin-only partner configuration, and P0-19 telemetry boundary are merged. Execution-fact safety now has a reviewed safe-phrase schema, fail-closed model bypass/grounding guard, and blocking runtime evals; local-address facts, legacy-address quarantine, deterministic POI alias resolution, and the no-fallback presentation guard are also merged. VisePod now has the accepted HTTPS Device Protocol v1 plus a draft Studio provisioning contract and pure domain schemas only; no Studio route, token issuer, binding table, device registry, gateway, or firmware runtime exists. ADR-0015 remains Proposed and ADR-0016/0017 remain Draft, so their formal status must be reconciled before dependent implementation expands. No partner is active, no booking or commission is promised, no payment is live, and Phase 1 through Phase 4 remain trigger-gated.
- **Last completed control action:** PRs #357/#358 defined the real-task research protocol and execution-fact safety boundary; #359/#360 added provenance-qualified local addresses and deterministic aliases; #362/#363/#366 added the safe-phrase contract, fail-closed grounding, and blocking safety evals; #364/#367 quarantined legacy addresses and guarded presentation; and #361 froze the draft VisePod Studio binding/provisioning contract without implementing runtime capability. Independent architecture review merged #361 as 264676c, whose main CI passed verify, evals, Database contracts, and docs. Parent Issues #344, #345, and #346 were closed only after every accepted child boundary merged. P0-19 remains terminal, production purge evidence remains verified under #311, and payment remains deferred under P0-17 / OA-006.

### Active Work

| Ref | Work | State | Owner | Next action |
| --- | --- | --- | --- | --- |
| Issue #157 / P0-20 | Add truthful public-runtime safety and budget controls | P0-19 is terminal and PR #338 is merged, but ADR-0015 still declares Status: Proposed. No merge result may silently promote that D2 contract to Accepted. | Independent architecture reviewer for ADR-0015; Codex implementation only after an explicit status verdict | Accept, revise, or reject ADR-0015 explicitly. Do not create a P0-20 runtime implementation branch while the authoritative ADR remains Proposed. |
| ADR-0016 / Issues #344-#351 | Reconcile execution-fact safety decision status | The safe-phrase schema, fail-closed pipeline, blocking evals, and parent #344 are complete, but ADR-0016 still declares Status: Draft. The implementation is safety-preserving; the document status is inconsistent with merged reality. | Independent architecture reviewer | Review the merged boundary and set ADR-0016 to Accepted or record a forward correction; do not weaken the shipped fail-closed controls while reconciling status. |
...
| --- | --- | --- |
| P0 | Reconcile the formal statuses of ADR-0015, ADR-0016, and ADR-0017 through independent architecture review. | Each ADR has an explicit Accepted, revised, or rejected verdict. P0-20 starts only after ADR-0015 is Accepted; VisePod Studio runtime starts only after ADR-0017 is Accepted; the merged ADR-0016 controls remain fail-closed during reconciliation. |
...
- P0-20 awaits an explicit architecture verdict on Proposed ADR-0015. Until it is Accepted or revised, D2 implementation must not start; the final boundary must preserve ADR-0005 truthful ready/degraded/unavailable semantics, reuse rather than weaken existing Copilot and telemetry guards, and avoid mock health or fake budget behavior.
- ADR-0016 and ADR-0017 remain Draft even though their bounded contract/implementation PRs merged. Independent architecture must reconcile those statuses before downstream work treats them as accepted authority.
...
| [ADR-0016: Execution-Fact Safety and High-Risk Fixed Expressions](adr/ADR-0016-execution-fact-safety.md) | decision | draft | AI and knowledge safety | Proposes fail-closed evidence and fixed-expression rules before Show to Local or other high-risk execution output is implemented. |
