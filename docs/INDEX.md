...
- **Updated:** 2026-07-28 by Codex / serialized post-#331/#332 telemetry refresh
- **Base branch:** `main`
- **Last fully verified commit:** `b535ecc feat(telemetry): rate-limit public capture; PR #332 head passed verify, Database contracts, evals, docs, and Vercel Preview Comments before authorized merge as 1968c13`
- **Current phase:** Phase 0 — production hardening and commercial safety
- **Maturity:** The real-provider Copilot, durable Trip completion, signed anonymous identity, cost and product-event persistence, three-turn and trusted-IP guards, truthful request states, controlled-preview Human Tasks, legal pages, retention contracts, database-local retention scheduler, durable outbound ledger, Admin-only partner configuration, P0-19 privacy-safe telemetry contract/adapter/views/producers/public-ingestion guard, and VisePod HTTPS Device Protocol v1 are merged. ADR-0013 freezes the Phase 0 Human Task rule: only durable intake may emit task_submitted; Ops triage/cancellation and quote/payment/done actions have no producer. The public telemetry endpoint now applies pre-write, HMAC-keyed dual identity/network Upstash limits and returns honest 429 or 503 outcomes. P0-20 is the next runtime-safety control action. No partner is active, no booking or commission is promised, no payment is live, and Phase 1 through Phase 4 remain trigger-gated.
- **Last completed control action:** PR #331 completed P0-19b by connecting only the accepted Copilot, Explore/Guide, durable outbound, and durable Human Task-intake producers; it kept payment and Ops-transition producers absent. PR #332 completed P0-19d with a Redis/Lua pre-write telemetry guard: 60/minute and 300/hour per identity plus 180/minute and 900/hour per trusted Vercel network by default, 429 plus Retry-After on exhaustion, and honest 503 when trusted Redis/HMAC dependencies are absent. Issue #156 is closed as terminal for its ADR-0013-scoped Phase 0 telemetry objective; payment conversion remains deferred with P0-17 / OA-006. PR #328 froze the VisePod v1 Wi-Fi HTTPS turn contract; PR #329 accepted ADR-0013; P0-18 remains complete through PRs #309/#315/#317. Issue #311 independently verified the three production migrations, pg_cron schedule, and retention health on VP-Final-V2 without a production write.

### Active Work

| Ref | Work | State | Owner | Next action |
| --- | --- | --- | --- | --- |
| Issue #157 / P0-20 | Add truthful public-runtime safety and budget controls | P0-19 is now terminal: the accepted telemetry contract, producer subset, and public-ingestion guard are merged. P0-20 may freeze and implement one coherent health, error, token-budget, and Human Help rate-control boundary without duplicating existing Copilot/telemetry limits or emitting unsupported payment lifecycle events. | Codex implementation; independent reviewer for Tier B contract and merge | Read the accepted runtime and telemetry ADRs, identify existing controls to reuse, freeze the smallest D2 health/budget/degradation/alert contract, then implement from fresh main in strictly serial PRs. |
...
| --- | --- | --- |
| P0 | Freeze and implement #157 / P0-20 from fresh main using ADR-0005, ADR-0012, the merged P0-19 controls, and one explicit Tier B runtime-safety contract. | P0-20 has explicit health/degradation, safe error-observation, non-duplicative request/token and Human Help limits, documented authorized Ops exemptions, verified truthful UI states, and an independently reviewed Tier B merge. |
...
- P0-20 is unblocked by merged P0-19, but its D2 contract must preserve ADR-0005 truthful ready/degraded/unavailable semantics, reuse rather than weaken existing Copilot and telemetry guards, and avoid mock health or fake budget behavior.
